...
export type RenderOptions = {
  ...
  weapon?: "fire"|"harpoon"|"mk45"|"radar"; selected?: Coord[]; active?: Coord[]; time?: number;
};
...
export function drawBoard(canvas: HTMLCanvasElement, board: Board, opts: RenderOptions) {
  ...
  if(opts.cursor){
    let cells=[opts.cursor]; if(opts.previewShip){const def=SHIPS.find(s=>s.id===opts.previewShip!.id)!;cells=board.cellsFor(opts.cursor,def.size,opts.previewShip.orientation);}
    else if(opts.weapon==="harpoon")cells=harpoonCells(opts.cursor); else if(opts.weapon==="radar")cells=radarCells(opts.cursor);
...
function drawShip(ctx:CanvasRenderingContext2D,id:ShipId,cells:Coord[],orientation:Orientation,m:number,cell:number,sunk:boolean,hits:Set<string>){
  const start=cells[0], horizontal=orientation==="horizontal";ctx.save();
