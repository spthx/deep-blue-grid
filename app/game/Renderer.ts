...
function drawShip(ctx:CanvasRenderingContext2D,id:ShipId,cells:Coord[],orientation:Orientation,m:number,cell:number,sunk:boolean,hits:Set<string>){
  ...
  if(id==="carrier"){
    ...
    const len=cell*4,w=cell*1.45;ctx.globalAlpha=sunk?.55:.9;ctx.fillStyle=sunk?"#584e51":"#71909b";ctx.beginPath();ctx.moveTo(-len*.48,-w*.42);ctx.lineTo(len*.38,-w*.48);ctx.lineTo(len*.5,-w*.22);ctx.lineTo(len*.5,w*.22);ctx.lineTo(len*.38,w*.48);ctx.lineTo(-len*.48,w*.42);ctx.closePath();ctx.fill();
    ctx.fillStyle=sunk?"#40383a":"#173b45";ctx.fillRect(-len*.36,-cell*.055,len*.72,cell*.11);ctx.fillRect(-cell*.05,-w*.38,cell*.12,w*.76);
    ctx.fillStyle=sunk?"#4a3b3e":"#b0ced0";ctx.fillRect(cell*.72,-w*.37,cell*.38,cell*.32);ctx.fillRect(cell*.82,-w*.52,cell*.1,cell*.18);
  ...
  const len=cells.length*cell,w=cell*.5;ctx.globalAlpha=sunk?.55:.88;ctx.fillStyle=sunk?"#584e51":"#71909b";
  ...
  ctx.fillStyle=sunk?"#4a3b3e":"#b0ced0";
