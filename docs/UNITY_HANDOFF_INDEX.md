...
[unity-content-v1.json](unity-handoff/unity-content-v1.json) を再生成する。配布物一覧は
...
| MISSION | `app/game/Campaign.ts`, `app/game/ExtremeMissions.ts`, `app/game/MissionRules.ts` | 固定状況・目的・制約・評価の再現 |
| INITIAL TRAINING | `app/game/Training.ts`, `app/game/TrainingRules.ts`, `app/game/TrainingProgress.ts` | 教材順序と進捗保存の再現 |
| 表現・レイアウト | `app/globals.css` | Safe Area 対応の uGUI + TextMeshPro |
...
- MISSION は自由選択の22任務：戦術12、ARCHIVE 4、EXTREME 6。任務間で艦損耗・
...
- CIC の低彩度ネイビー、cyan/amber/danger、scanline/noise、低振幅の ECHO・HIT・
  SUNK pulse を再現する。点滅を強くしたり、Reduce Motion で情報を消したりしない。
...
3. [MISSION library spec](MISSION_LIBRARY_SPEC.md) — 22任務のカテゴリ、公開情報、
   制約、EXTREME一覧、静的検証。
...
9. [Mission clearability report](MISSION_CLEARABILITY_REPORT.md) — 22本の release-screen
...
- [ ] CASUAL / TACTICS 6海域、SURVIVAL 4作戦、22 MISSION、6教程を開始・リトライ・
  終了まで通す。22 canonical route はすべて成功する。
- [ ] 22任務の `PLOTTED CONTACTS`、ARCHIVE日誌、順序付き報告、兵装順・使用数、
  撃沈順、保護艦敗北を同じ条件で評価する。
...
6. MISSION 22任務、ARCHIVE日誌、EXTREME制約、INITIAL TRAINING 6教程と進捗を追加する。
