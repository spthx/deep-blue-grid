...
test("all sixteen freely selected missions keep independent records", () => {
  let records = createEmptyMissionRecords();
  for (let missionId = 16; missionId >= 1; missionId -= 1) {
    records = applyMissionResult(records, {
      missionId,
      result: "victory",
      commands: missionId,
  ...
  assert.equal(records.missions.length, 16);
  for (let missionId = 1; missionId <= 16; missionId += 1) {
