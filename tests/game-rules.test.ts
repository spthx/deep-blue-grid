...
import { SHIPS } from "../app/game/constants.ts";
import { Arsenal, Board, SeededRandom, harpoonCells, radarCells } from "../app/game/engine.ts";
...
test("AI never repeats or leaves the board over a full simulated hunt", () => {
  ...
  assert.ok(ai.arsenal.uses.harpoon >= 0 && ai.arsenal.uses.mk45 >= 0 && ai.arsenal.uses.radar >= 0);
});
