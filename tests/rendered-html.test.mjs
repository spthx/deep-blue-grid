...
test("mission mode exposes a free-select library, archive review, records, and thumb-reachable command surface", async () => {
  ...
  assert.match(game, /type MissionCompletionFilter = "all" \| "uncleared" \| "cleared"/);
  assert.match(game, /aria-label="達成状況で任務を絞り込む"/);
  ...
  assert.match(game, /EXTREME OPERATIONS/);
  assert.match(game, /TRAINING DIRECTIVE/);
  assert.match(game, /updateStoredTrainingProgress/);
  assert.match(game, /matchesTrainingStep/);
  assert.match(game, /PLOTTED CONTACTS \/ 公開座標/);
  assert.match(game, /mission-public-intel/);
  assert.match(game, /trainingAllComplete/);
  assert.match(game, /nextTrainingIndex/);
  assert.match(game, /hasNextTrainingLesson/);
  assert.match(game, /trainingHoldNotice/);
  assert.match(game, /行動・弾薬は消費していません/);
  assert.match(game, /NO MATCHING OPERATIONS/);
  ...
  assert.match(css, /\.mission-library-filters/);
  assert.match(css, /\.mission-card\.uncleared \{ border-left-style:dashed/);
  assert.match(css, /\.mission-card \.mission-status-badge\.cleared/);
  assert.match(css, /\.mission-difficulty/);
  assert.match(css, /\.mode-button\.training/);
  assert.match(css, /\.training-directive/);
  assert.match(css, /\.training-hold-notice/);
  assert.match(css, /\.mission-card\.extreme/);
  assert.match(css, /\.mission-public-intel/);
  assert.match(css, /\.mission-mode \.compact-placement-bottom \.mission-orders[\s\S]*?grid-column:1; grid-row:2/);
  assert.match(css, /\.mission-mode \.compact-placement-bottom \.placement-start[\s\S]*?position:static; grid-column:2; grid-row:2/);
...
test("CIC logs use Zulu timestamps and extreme defeat review preserves retry intelligence", async () => {
  const game = await readFile(new URL("../app/game/DeepBlueGrid.tsx", import.meta.url), "utf8");
  const css = await readFile(new URL("../app/globals.css", import.meta.url), "utf8");
  ...
  assert.match(game, /INTELLIGENCE WITHHELD/);
  assert.match(game, /再挑戦資格保全/);
  assert.match(game, /交戦後解析：敵配置確認/);
  assert.match(game, /revealShips: phase === "defeat" && resultReview && !extremeIntelWithheld/);
  assert.match(game, /concealDamage: identificationRules && \(!\(phase === "defeat" && resultReview\) \|\| extremeIntelWithheld\)/);
